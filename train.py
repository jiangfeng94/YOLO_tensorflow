...
class Solver(object):
	def _init_(self,net,data):
		...
		self.decay_rate =cfg.DECAY_RAET
		self.staircase =cfg.STAIRCASE 
		self.summary_iter =cfg.SUMMARY_TIER 
		...
		self.learning_rate = tf.trian.exponential_decay(self.initial_learning_rate,self.global_step,self.decay_steps,self.decay_rate,
			self.staircase,name ='learning rate')
		...
		for step in range(1,self.max_iter +1):
			...
			if step % self.summary_iter ==0:
				if step %(self.summary_iter *10) ==0:
					train_timer.tic()
					summary_iter,loss,_ =self.sess.run(
...
def update_config_paths(data_dir,weights_file):
	...
	cfg.PASCAL_VOC =os.path.join(data_dir,'pascal_voc')
	cfg.CACHE_PATH = os.path.join(cfg.PASCAL_VOC,'cache')
	cfg.OUTPUT_DIR=os.path.join(cfg.PASCAL_VOC,'output')
	cfg.WEIGHTS_DIR =os.path.join(cfg.PASCAL_VOC,'weights')
	cfg.WEIGHTS_FILE =os.path.join(cfg.PASCAL_VOC,weights_file)
def main():
   ...
   #设置YOLONet 的建立是通过 yolo文件夹中的 yolo_net.py实现。
	yolo =YOLONet()
   #读取训练数据
